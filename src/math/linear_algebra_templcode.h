//---------------------------------------------------------------------------
#ifndef linear_algebra_templcode_h_
#define linear_algebra_templcode_h_
...
inline FloatType& StaticVector<FloatType, dim>::operator[](const unsigned &index) {
   #ifdef RANGE_CHECK
   if (index >= dim) {
      cout<<"StaticVector::operator[] - index exceeds bounds"<<endl;
	  exit(1);
   }
   #endif
   return v[index];
...
inline StaticVector<FloatType, rows>& StaticMatrix<FloatType, columns, rows>::operator[](const unsigned &index) {
   #ifdef RANGE_CHECK
   if (index >= columns) {
      cout<<"StaticMatrix::operator[] - index exceeds bounds"<<endl;
	  exit(1);
   }
   #endif
